...
class Config:
    def __init__(self):
        ...
        self.google_cloud_project = self._get_optional_env("GOOGLE_CLOUD_PROJECT")
        self.google_cloud_location = (
            self._get_optional_env("GOOGLE_CLOUD_LOCATION") or "global"
        )
        self.google_genai_use_vertexai = self._get_bool_env(
        ...
        self.resend_from_email = os.getenv(
            "RESEND_FROM_EMAIL", "AI Shorts Gen <onboarding@resend.dev>"
        )
        self.app_base_url = (
    ...
    def has_google_vertex_credentials(self) -> bool:
